...
def visualize_predictions(generator, dataloader, device, max_images=4, title="Colorization", random_samples=False):
    """
    Show:
    ...
    if random_samples:
        indices = torch.randperm(len(dataloader.dataset))[:max_images].tolist()
        samples = [dataloader.dataset[index] for index in indices]
        L_batch = torch.stack([sample[0] for sample in samples], dim=0)
        lab_batch = torch.stack([sample[1] for sample in samples], dim=0)
    else:
